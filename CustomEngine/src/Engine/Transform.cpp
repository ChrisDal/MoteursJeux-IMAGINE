...
Transform::Transform(const glm::mat4x4& mat4)
	: m_transform(mat4)
{
	m_translation = glm::vec3(mat4[0, 3], mat4[1, 3], mat4[2, 3]);
	m_rotation = Rotoform();
...
inline void Transform::processMat4x4Transform()
{
	...
	// 3. apply translation
	m_transform[0][3] = m_translation[0];
	m_transform[1][3] = m_translation[1];
	m_transform[2][3] = m_translation[2];
...
{
	m_scale[0] = scale_factor;
	m_scale[1] = scale_factor;
	m_scale[2] = scale_factor;
}
...
{
	m_translation[0] = txyz;
	m_translation[1] = txyz;
	m_translation[2] = txyz;
}
...
void Transform::applyToMatrix(glm::mat4x4& MatIn)
{
	MatIn = m_transform * MatIn;
}

// Matrix multiplication4x4
glm::mat4x4 Transform::Multiply(const glm::mat4x4& MatIn)
{
...
// Matrix multiplication4x4
glm::mat4x4 Transform::combine_with(Transform MatIn)
{
	glm::mat4x4 combined = this->getMatrixTransform() * MatIn.getMatrixTransform();
...
glm::mat4x4 Transform::getMatrixTransform() {
	this->processMat4x4Transform();
	return m_transform;
}
glm::mat4x4 Transform::getTransposedTransform() {
...
std::ostream& operator<<(std::ostream& stream, const Transform& transfo)
{
	stream << "Translation : " << transfo.m_translation.x << "," << transfo.m_translation.y << "," << transfo.m_translation.z << std::endl;
	stream << "Scaling : " << transfo.m_scale.x << "," << transfo.m_scale.y << "," << transfo.m_scale.z << std::endl;
	stream << "Rotation : " << transfo.m_rotation << std::endl;
